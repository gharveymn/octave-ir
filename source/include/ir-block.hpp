...
namespace octave
{
  ...
  public:
    ...
    public:
      ...
      [[nodiscard]] iter   begin   (void)       noexcept { return m_timeline.begin ();   }
      [[nodiscard]] citer  begin   (void) const noexcept { return m_timeline.begin ();   }
      [[nodiscard]] citer  cbegin  (void) const noexcept { return m_timeline.cbegin ();  }

      [[nodiscard]] iter   end     (void)       noexcept { return m_timeline.end ();     }
      [[nodiscard]] citer  end     (void) const noexcept { return m_timeline.end ();     }
      [[nodiscard]] citer  cend    (void) const noexcept { return m_timeline.cend ();    }

      [[nodiscard]] riter  rbegin  (void)       noexcept { return m_timeline.rbegin ();  }
      [[nodiscard]] criter rbegin  (void) const noexcept { return m_timeline.rbegin ();  }
      [[nodiscard]] criter crbegin (void) const noexcept { return m_timeline.crbegin (); }

      [[nodiscard]] riter  rend   (void)       noexcept { return m_timeline.rend ();     }
      [[nodiscard]] criter rend   (void) const noexcept { return m_timeline.rend ();     }
      [[nodiscard]] criter crend  (void) const noexcept { return m_timeline.crend ();    }
      
      [[nodiscard]]
      constexpr bool has_cache (void) const noexcept 
      {
        return m_lookback.has_remote ();
      }
      
      [[nodiscard]]
      constexpr ir_def& fetch_cache (void) const noexcept
      {
        return m_lookback.get_remote ();
      }

      void set_cache (ir_def& latest) noexcept
      {
        m_lookback.rebind (latest);
      }
    ...
    private:
      ...
      gch::standalone_reporter<ir_def> m_lookback;
      
      std::pair<phi_citer, gch::optional_ref<ir_def>> m_phi_def;
  ...
  public:
    ...
    gch::optional_ref<ir_def> fetch_cached_def (ir_variable& var) const;

    gch::optional_ref<ir_def> 
    fetch_proximate_def (ir_variable& var, instr_citer pos) const;
    ...
    instr_citer  cend (void)    const noexcept { return m_instrs.cend ();    }
...
//    instr_criter body_rend (void)   const noexcept { return instr_criter (body_begin ()); }
    ...
    template <typename T>
    using enable_ret_nonphi
      = std::enable_if_t<std::conjunction<is_nonphi<T>, 
                                               has_return<T>>::value>;
    ...
    T& emplace_before (instr_citer pos, Args&&... args)
    {
      ...
      std::unique_ptr<T> u 
        = create_instruction<T> (std::forward<Args> (args)...);
    ...
    { 
      return link_iter (value_begin ()); 
    }

    link_iter leaf_end (void) noexcept override 
    { 
      return link_iter (value_end ()); 
    }
    
    value_iter<ir_basic_block *> value_begin (void) noexcept 
    { 
      return value_iter<ir_basic_block *> (this); 
    }

    value_iter<ir_basic_block *> value_end (void) noexcept
    {
      return ++value_begin ();
    }
