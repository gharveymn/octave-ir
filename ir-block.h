...
namespace octave
{
  ...
  private:
    ...
    public:
      ...
      void track_phi_def (phi_citer pos, def& d)
      {
        if (has_phi_def ())
    ...
    private:
      ...
      std::pair<phi_citer, def *> m_phi_def = {{}, nullptr};
  ...
  public:
    ...
    def * fetch_proximate_def (ir_variable& var, instr_citer pos) const;
    
    // side effects!
    def * join_defs (ir_variable& var);
    
    // side effects!
    def * join_defs (ir_variable& var, instr_citer pos);
    ...
    instr_iter
    body_begin (void)        noexcept { return m_body_begin; }
    instr_citer
    body_begin (void)  const noexcept { return m_body_begin; }

    instr_iter
    body_end (void)          noexcept { return m_terminator;   }
    instr_citer
    body_end (void)    const noexcept { return m_terminator;   }

    instr_riter
    body_rbegin (void)       noexcept { return instr_riter (body_end ()); }
    instr_criter
    body_rbegin (void) const noexcept { return instr_criter (body_end ()); }

    instr_riter
    body_rend (void)         noexcept { return instr_riter (body_begin ()); }
    instr_criter
    body_rend (void)   const noexcept { return instr_criter (body_begin ()); }
  ...
  protected:
    ...
    void def_emplace (instr_citer pos, def& d);
